 ...
 * - Anonymous Reactive Functions for children's items
...
"use strict";
const jurisLinesOfCode = 2626;
const jurisVersion = "0.9.0";
const jurisMinifiedSize = "45.11 kB";
     ...
     #notifyExternalSubscribers(changedPath, newValue, oldValue) {
          this.externalSubscribers.forEach((subscriptions, subscribedPath) => {
               subscriptions.forEach(({ callback, hierarchical }) => {
                    const shouldNotify = hierarchical ? changedPath === subscribedPath || changedPath.startsWith(subscribedPath + ".") : changedPath === subscribedPath;
                    if (shouldNotify) {
                         try {
...
class ComponentManager {
     ...
     create(name, props = {}) {
          ...
          try {
               ...
               return this.#createErrorElement(new Error(error.message + " Did you forgot to use children:[]?"));
          }
     }
     ...
     #createComponentContext(componentId, componentStates) {
          ...
          context.newState = (key, initialValue) => {
               ...
               return [() => this.juris.stateManager.getState(statePath, initialValue), (value) => this.juris.stateManager.setState(statePath, value)];
          };
          return context;
     }
     ...
     #processComponentResult(result, name, props, componentStates) {
          if (Array.isArray(result)) {
               ...
               container.setAttribute("data-juris-reactive-array", name);
               const componentData = { name, props };
               this.instances.set(container, componentData);
               const subscriptions = [];
               this.juris.domRenderer._handleChildrenFineGrained(container, result, subscriptions);
     ...
     #hasLifecycleHooks(result) {
          return (result.hooks && (result.hooks.onMount || result.hooks.onUpdate || result.hooks.onUnmount)) || result.onMount || result.onUpdate || result.onUnmount;
     }
     ...
     getComponent(name) {
          return this.namedComponents.get(name)?.instance || null;
     }
     getComponentAPI(name) {
          return this.namedComponents.get(name)?.instance?.api || null;
     }
     getComponentElement(name) {
          return this.namedComponents.get(name)?.element || null;
     }
     getNamedComponents() {
          return Array.from(this.namedComponents.keys());
     }
     updateInstance(element, newProps) {
          ...
          if (this.juris.domRenderer._hasAsyncProps(newProps)) {
               this.#resolveAsyncProps(newProps)
                    .then((resolvedProps) => {
...
class DOMRenderer {
     ...
     #createElementStatic(tagName, props, componentName = null) {
          const element = this.SVG_ELEMENTS.has(tagName.toLowerCase()) ? document.createElementNS("http://www.w3.org/2000/svg", tagName) : document.createElement(tagName);
     ...
     #reconcileChildren(parent, oldChildren, newChildren) {
          console.debug(
               log.d(
                    ...
                    { parentTag: parent.tagName, oldCount: oldChildren.length, newCount: Array.isArray(newChildren) ? newChildren.length : newChildren ? 1 : 0 },
                    "framework"
               )
          );
          ...
          promisify(stylePromise)
               .then((resolvedStyle) => {
                    ...
                    if (typeof resolvedStyle === "object") {
                         if (config.style) {
                              const loadingProps = config.style
                                   .split(";")
     ...
     #handleReactiveStyle(element, styleFn, subscriptions) {
          ...
          const updateStyle = () => {
               try {
                    ...
                    // CRITICAL FIX: Post-process with CSSExtractor
     ...
     #createElementOptimized(tagName, props, key) {
          ...
          if (!element) {
               element = this.SVG_ELEMENTS.has(tagName.toLowerCase()) ? document.createElementNS("http://www.w3.org/2000/svg", tagName) : document.createElement(tagName);
          }
          if (key) {
     ...
     _handleEvent(element, eventName, handler, eventListeners) {
          ...
          if (eventName === "onclick") {
               ...
               eventListeners.push({ eventName: "touchstart", handler: touchStart }, { eventName: "touchmove", handler: touchMove }, { eventName: "touchend", handler: touchEnd });
          } else {
     ...
     _setStaticAttribute(element, attr, value) {
          ...
          if (
               this.PRESERVED_ATTRIBUTES.has(attr) ||
     ...
     cleanup(element) {
          console.debug(log.d("Cleaning up element", { tagName: element.tagName }, "framework"));
          ...
          if (data) {
               if (data.subscriptions) {
                    data.subscriptions.forEach((unsub) => {
                         try {
                              unsub();
                         } catch (e) {}
                    });
...
class Juris {
     ...
     constructor(config = {}) {
          ...
          if (features.headless) {
               this.headlessManager = new features.headless(this);
               this.headlessAPIs = {};
          }
          if (features.enhance) {
          ...
          if (features.cssExtract) {
               this.domRenderer.customCSSExtractor = new features.cssExtract();
          }
     ...
     #detectGlobalAndWarn() {
          if (!Juris._done) {
               (requestIdleCallback || setTimeout)(() => {
                    if (Juris.#inGlobal) return;
                    Juris.#inGlobal = true;
                    for (let key in globalThis) {
                         if (globalThis[key] instanceof Juris) {
                              console.warn(`âš ï¸ JURIS GLOBAL: '${key}'`);
                         }
                    }
               });
          }
     ...
     getComponentAPI(name) {
          return this.componentManager.getComponentAPI(name);
     }
     getComponentElement(name) {
          return this.componentManager.getComponentElement(name);
     }
     getNamedComponents() {
          return this.componentManager.getNamedComponents();
     }

     compileTemplates() {
          if (!this.templateCompiler) {
               console.warn(log.w("Template compilation requested but templateCompiler not available"), "framework");
               return;
          }
     ...
     createWebComponent(name, componentDefinition, options = {}) {
          if (!this.webComponentFactory) {
               console.error(log.e("WebComponent creation requested but webComponentFactory not available"), "application");
               return null;
          }
     ...
     createWebComponents(components, globalOptions = {}) {
          if (!this.webComponentFactory) {
               console.error(log.e("WebComponents creation requested but webComponentFactory not available"), "framework");
               return {};
          }
     ...
     getState(path, defaultValue, track) {
          return this.stateManager.getState(path, defaultValue, track);
     }
     setState(path, value, context) {
     ...
     configureEnhancement(options) {
          if (!this.domEnhancer) {
               console.warn(log.w("Enhancement configuration requested but domEnhancer not available"), "framework");
               return;
          }
     ...
     cleanup() {
          console.info(log.i("Framework cleanup initiated", {}, "faramework"));
          this.headlessManager?.cleanup();
     }

     destroy() {
          console.info(log.i("Framework destruction initiated", {}, "faramework"));
          this.cleanup();
          if (this.domEnhancer) {
          ...
          console.info(log.i("Framework destroyed", {}, "faramework"));
     }
}
