     ...
     reinitialize(name, props = {}) {
          ...
          if (instance?.hooks?.onUnregister) {
               try {
                    ...
                    console.error(log.e(`Error in onUnregister for '${name}':`, error), "framework");
               }
          }
...
class ComponentManager {
     ...
     _handleAsyncRender(renderPromise, name, componentStates, indicator = null) {
          const tempElement = document.createElement("div");
          tempElement.id = name.toLowerCase().replace(/[^a-z0-9]/g, "-");
          const placeholder = indicator ? this.juris.domRenderer.render(indicator) : this._createPlaceholder(`Loading ${name}...`, "juris-async-loading", tempElement);
          renderPromise
               .then((renderResult) => {
                    try {
                         const element = this.juris.domRenderer.render(renderResult);
     ...
     updateInstance(element, newProps) {
          ...
          if (this._hasAsyncProps(newProps)) {
               this._resolveAsyncProps(newProps)
                    .then((resolvedProps) => {
...
class TemplateCompiler {
     ...
     convertElement(element) {
          ...
          const processedChildren = children.map((child) => this.convertNode(child)).filter((child) => child !== null);
     ...
     generateComponent(parsed) {
          ...
          const combinedScript = contextDestructuring ? `${contextDestructuring}\n${parsed.script}` : parsed.script;
...
return ${objStr};
     ...
     objectToString(obj, indent = 0) {
          ...
          if (Array.isArray(obj)) {
               ...
               const items = obj.map((item) => spaces + "  " + this.objectToString(item, indent + 1)).join(",\n");
               return "[\n" + items + "\n" + spaces + "]";
          }
...
class DOMEnhancer {
     ...
     _enhanceViewportAware(selectorOrElement, definition, config) {
          console.debug(
               log.d(
                    "Viewport-aware enhancement starting",
                    {
     ...
     _processPendingViewportChanges() {
          const viewportChanges = Array.from(this.pendingEnhancements)
               ...
               .map((item) => item.entry);
          this.pendingEnhancements = new Set(Array.from(this.pendingEnhancements).filter((item) => item.type !== "viewport"));
     ...
     _observeElementForViewport(element, definition, config) {
          ...
          const isInitiallyVisible = rect.top < window.innerHeight && rect.bottom > 0 && rect.left < window.innerWidth && rect.right > 0;
     ...
     _batchEnhanceElements(elements, definition, config) {
          elements.filter((element) => !this.enhancedElements.has(element)).forEach((element) => this._enhanceElement(element, definition, config));
     }
...
class WebComponentFactory {
     ...
     createMultiple(components, globalOptions = {}) {
          ...
          Object.entries(components).forEach(([name, definition]) => {
               const options = definition.options ? { ...globalOptions, ...definition.options } : globalOptions;
     ...
     _createWebComponentClass(name, componentDefinition, options) {
          ...
          const { shadowMode = "open", attributes = [], styles = "", enhanceMode = false, autoConnect = true, stateNamespace = null, contextProvider = null } = options;
          ...
          return class JurisWebComponent extends HTMLElement {
               ...
               _createJurisContext() {
                    const baseContext = contextProvider ? contextProvider.call(this, jurisInstance.createContext(this)) : jurisInstance.createContext(this);
                    return {
                         ...baseContext,
                         component: {
                              ...
                              getSlot: (name = "") => {
                                   return name ? this.querySelector(`[slot="${name}"]`) : this.querySelector(":not([slot])");
                              },
                              getAllSlots: () => {
                                   const slots = {};
...
class Juris {
     ...
     constructor(config = {}) {
          ...
          console.info(
               log.i("Juris framework initialized", { componentsCount: this.componentManager.components.size, headlessCount: this.headlessManager.components.size }, "framework")
          );
     ...
     _detectGlobalAndWarn() {
          if (!Juris._done) {
               (requestIdleCallback || setTimeout)(() => {
                    if (Juris._inGlobal) return;
                    Juris._inGlobal = true;
                    for (let key in globalThis) {
                         if (globalThis[key] instanceof Juris) {
                              console.warn(`⚠️ JURIS GLOBAL: '${key}'`);
                         }
                    }
               });
          }
